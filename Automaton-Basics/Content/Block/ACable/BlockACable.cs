...
namespace Automaton.Content.Block.ACable
{
    public class BlockACable : BlockEBase
    {
        ...
        private static readonly Dictionary<(Facing main, Facing sub), (float x, float y, float z)> RotationMap = new()
        {
            // North
            { (Facing.NorthAll, Facing.NorthEast), (90, 270, 0) },
            { (Facing.NorthAll, Facing.NorthWest), (90, 90, 0) },
            { (Facing.NorthAll, Facing.NorthUp),   (90, 0, 0) },
            { (Facing.NorthAll, Facing.NorthDown), (90, 180, 0) },

            // East
            { (Facing.EastAll, Facing.EastNorth), (0, 0, 90) },
            { (Facing.EastAll, Facing.EastSouth), (180, 0, 90) },
            { (Facing.EastAll, Facing.EastUp),    (90, 0, 90) },
            { (Facing.EastAll, Facing.EastDown),  (270, 0, 90) },

            // South
            { (Facing.SouthAll, Facing.SouthEast), (270, 270, 0) },
            { (Facing.SouthAll, Facing.SouthWest), (270, 90, 0) },
            { (Facing.SouthAll, Facing.SouthUp),   (270, 180, 0) },
            { (Facing.SouthAll, Facing.SouthDown), (270, 0, 0) },

            // West
            { (Facing.WestAll, Facing.WestNorth), (0, 0, 270) },
            { (Facing.WestAll, Facing.WestSouth), (180, 0, 270) },
            { (Facing.WestAll, Facing.WestUp),    (90, 0, 270) },
            { (Facing.WestAll, Facing.WestDown),  (270, 0, 270) },

            // Up
            { (Facing.UpAll, Facing.UpNorth), (0, 0, 180) },
            { (Facing.UpAll, Facing.UpEast),  (0, 270, 180) },
            { (Facing.UpAll, Facing.UpSouth), (0, 180, 180) },
            { (Facing.UpAll, Facing.UpWest),  (0, 90, 180) },

            // Down
            { (Facing.DownAll, Facing.DownNorth), (0, 0, 0) },
            { (Facing.DownAll, Facing.DownEast),  (0, 270, 0) },
            { (Facing.DownAll, Facing.DownSouth), (0, 180, 0) },
            { (Facing.DownAll, Facing.DownWest),  (0, 90, 0) },
        };
        ...
        {
            return base.IsReplacableBy(block) || block is BlockACable;
        }
        ...
        public override bool DoPlaceBlock(IWorldAccessor world, IPlayer byPlayer, BlockSelection blockSelection, ItemStack byItemStack)
        {
            ...
            if ((entity.Connection & facing) == 0)  //мы навелись уже на существующий кабель?
            {
                ...
                else   //линий не 0, значит уже что-то там есть на грани
                {
                    ...
                    var block = new GetCableAsset().CableAsset(api, entity.Block, indexM2, 1); // берем ассет блока кабеля
        ...
        public override void OnBlockBroken(IWorldAccessor world, BlockPos position, IPlayer byPlayer, float dropQuantityMultiplier = 1)
        {
            ...
            foreach (var face in FacingHelper.Faces(selectedFacing))
            {
                ...
                var block = new GetCableAsset().CableAsset(api, entity.Block, material, 1);
        ...
        public override void OnNeighbourBlockChange(IWorldAccessor world, BlockPos pos, BlockPos neibpos)
        {
            ...
            foreach (var face in FacingHelper.Faces(selectedConnection))         //перебираем все грани выделенных кабелей
            {
                ...
                var block = new GetCableAsset().CableAsset(api, entity.Block, material, 1); //берем ассет блока кабеля
        ...
        {
            if (blockAccessor.GetBlockEntity(position) is BlockEntityACable { AllAparams: not null } entity)
            {
                var key = CacheDataKey.FromEntity(entity);
        ...
        {
            if (api.World.BlockAccessor.GetBlockEntity(position) is BlockEntityACable entity
                && entity.Connection != Facing.None
                && entity.AllAparams != null
                && entity.Block.Code.ToString().Contains("acable"))
            {
                var key = CacheDataKey.FromEntity(entity);
                ...
                if (!MeshDataCache.TryGetValue(key, out var meshData))
                {
                    ...
                    foreach (var (main, sub) in RotationMap.Keys.Where(k => (key.Connection & k.sub) != 0))
                    {
                        int bufIndex = FacingHelper.Faces(main).First().Index;
                        var eparam = entity.AllAparams![bufIndex];
                        var partVariant = new BlockVariants(api, entity.Block, eparam.material, 1);
        ...
        public static Dictionary<Facing, Cuboidf[]> CalculateBoxes(CacheDataKey key,
            IDictionary<CacheDataKey, Dictionary<Facing, Cuboidf[]>> boxesCache, BlockEntityACable entity)
        {
                ...
                && entity.Block.Code.ToString().Contains("acable"))
            {
                ...
                foreach (var (main, sub) in RotationMap.Keys.Where(k => (key.Connection & k.sub) != 0))
                {
                    int bufIndex = FacingHelper.Faces(main).First().Index;
                    var eparam = entity.AllAparams![bufIndex];
                    var partBoxes = new BlockVariants(entity.Api, entity.Block, eparam.material, 1).CollisionBoxes;

                    var (x, y, z) = RotationMap[(main, sub)];
                    var rotated = partBoxes.Select(b => b.RotatedCopy(x, y, z, origin)).ToArray();

                    AddBoxes(ref boxes, sub, rotated);
                }
            }

            // если это не кабель, возвращаем стандартные коллайдеры
            if (!entity.Block.Code.ToString().Contains("acable"))
            {
